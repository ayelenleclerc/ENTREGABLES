...
export default class ProductManager {
  ...
  addProduct = async (product) => {
    try {
      ...
      if (codeRepeat) {
        return console.log("The insert code already exists");
      }
      let id;
  ...
  updateProduct = async (product) => {
    try {
      ...
      if (!productUpdate) {
        return console.log(`Can't find product with id : ${product.id}`);
      }
  ...
  deleteProduct = async (id) => {
    try {
      ...
      if (index < 0) {
        return console.log(`Can't find product with id : ${id}`);
      }
